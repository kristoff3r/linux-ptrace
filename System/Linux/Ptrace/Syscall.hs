{-# LANGUAGE ForeignFunctionInterface, GeneralizedNewtypeDeriving, CPP, MagicHash #-}
...
import Foreign hiding (WordPtr, ptrToWordPtr, wordPtrToPtr)
import Foreign.C
...
-- Work around https://ghc.haskell.org/trac/ghc/ticket/11983 by copying the WordPtr definition
import GHC.Ptr
import GHC.Base
import GHC.Num
import GHC.Read
...
instance Read WordPtr where
   readsPrec            = unsafeCoerce# (readsPrec :: Int -> ReadS Word)
   readList             = unsafeCoerce# (readList  :: ReadS [Word])
instance Show WordPtr where
   showsPrec            = unsafeCoerce# (showsPrec :: Int -> Word -> ShowS)
   show                 = unsafeCoerce# (show :: Word -> String)
   showList             = unsafeCoerce# (showList :: [Word] -> ShowS)
